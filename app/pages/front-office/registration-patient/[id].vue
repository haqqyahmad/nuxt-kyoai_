  ...
  <UDashboardPanel :id="`registration-${route.params.id}`">
    <template #header>
      <UDashboardNavbar title="Detail Registrasi">
        <template #leading>
          <UButton icon="i-lucide-arrow-left" color="neutral" variant="ghost" to="/front-office/registration-patient" />
        </template>
        <template #right>
          <div class="flex items-center gap-2">
            <UButton icon="i-lucide-printer" color="neutral" variant="outline" label="Print Label" />
            <UButton
              v-if="!isCancelled"
              icon="i-lucide-x-circle" color="error" variant="outline"
              label="Cancel Registrasi" :loading="cancelLoading"
              @click="cancelRegistration"
            />
            <UButton
              v-if="!isCancelled && !isCheckedIn"
              icon="i-lucide-user-check" color="primary"
              label="Check-in Pasien" :loading="checkinLoading"
                  ...
                  >
                    ...
                    <td class="px-5 py-3 text-right">
                      <div class="flex justify-end gap-1">
                        <UButton
                          icon="i-lucide-eye" color="primary" variant="ghost" size="xs"
                          :disabled="q.status !== 'Completed'"
                          @click="q.status === 'Completed' && openModal(q.name)"
                        />
                        <UButton
                          icon="i-lucide-printer" color="primary" variant="ghost" size="xs"
                          :disabled="q.status !== 'Completed'"
                        />
                      </div>
                    </td>
