...
type Company = { id: number, codeCostumer: string, CustomerType: string, customerName: string }
...
watch(patientSearch, (val) => {
  ...
  debounce = setTimeout(async () => {
    try {
      ...
      console.log('pasien', r.data?.data?.data)
      patientResults.value = r.data?.data?.data ?? []
    } catch {
      patientResults.value = []
    } finally {
...
const { data: companies, pending: companiesPending, error } = await useAsyncData(
  'companies',
  async () => {
    try {
            ...
            <div class="rounded-xl border border-default overflow-visible">
              ...
              <div class="p-4 relative">
                ...
                >
                  ...
                  />
                  <span class="flex-1 truncate">{{ selectedBranch.nameBranch }} ·
              ...
              <div class="p-4 space-y-4">
                <UFormField label="Perusahaan / Pembayar *">
                  <USelect
                    ...
                    :items="
                      (companies ?? []).map((c) => ({
                        label: `${c.CustomerType}. ${c.customerName}`,
                        value: String(c.id)
                      }))
                    "
