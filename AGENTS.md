...
- `sample-collection/[id].vue`: enhance major — tambah room session management (enter/exit), "Ambil Pasien" modal (waiting list), "Mulai Pemeriksaan" (stage start), "Selesaikan" (stage done saat semua sample final), sidebar collapse, hapus duplicate UAlert.
...
- Flow: self-assign ke LAB/LAB-MCU → redirect sample-collection → masuk room → ambil pasien → mulai pemeriksaan → ambil/tolak/reschedule per sample → selesaikan.
